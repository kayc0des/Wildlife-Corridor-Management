...
class WildlifeCorridorEnv(gym.Env):
    ...
    def __init__(self, render_mode=None):
        ...
        self.agent_pos = self.start_pos  # Initialize as a tuple
    ...
    def reset(self, seed=None, options=None):
        ...
        self.agent_pos = list(self.start_pos)  # Ensure it's a tuple
    ...
    def step(self, action):
        ...
        # Ensure agent_pos is a tuple
        agent_pos = tuple(self.agent_pos)  # Explicitly cast to tuple
        new_pos = (
            agent_pos[0] + moves[action][0],
            agent_pos[1] + moves[action][1],
        )

        # Ensure the new position is within bounds of the grid
        new_pos = (
            max(0, min(self.grid_size - 1, new_pos[0])),
            max(0, min(self.grid_size - 1, new_pos[1])),
        )

        # Print debugging information to check types
        print(f"agent_pos: {agent_pos}, new_pos: {new_pos}")
        ...
        if new_pos in self.obstacles:
            ...
            self.goal_pos = None  # Remove goal after reaching
        else:
        ...
        terminated = self.goal_pos is None
        truncated = False  # Set to True if you want to include truncation logic

        # Return observation, reward, terminated, truncated, info
        observation = np.array(self.agent_pos, dtype=np.int32)  # Ensures return type is consistent
